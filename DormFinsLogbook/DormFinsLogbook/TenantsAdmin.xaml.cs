...
namespace DormFinsLogbook
{
    public partial class TenantsAdmin : Page
    {
        ...
        private void btnDeleyeTenant_Click(object sender, RoutedEventArgs e)
        {
            if (TenantsList.SelectedItem is Tenant selectedTenant)
            {
                if (MessageBox.Show($"Вы уверены, что хотите удалить жильца {selectedTenant.TenantFullName}?", "Подтверждение удаления", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
